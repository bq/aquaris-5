...
{
   PRINTK("jacob  Sound_SpeakerL_SetVolLevel level=%d\n",level);
}

void Sound_SpeakerR_SetVolLevel(int level)
{
   PRINTK("jacob  Sound_SpeakerR_SetVolLevel level=%d\n",level);
}

void Sound_Speaker_Turnon(int channel)
{
    PRINTK("jacob  Sound_Speaker_Turnon channel = %d,gsk_on =%d\n",channel,gsk_on);
	if(gsk_on)
		return;
	PRINTK("jacob 11  Sound_Speaker_Turnon channel = %d,gsk_on =%d\n",channel,gsk_on);
    mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
...
{
    PRINTK("jacob Sound_Speaker_Turnoff channel = %d\n",channel);
	if(!gsk_on)
		return;
	PRINTK("jacob 11 Sound_Speaker_Turnoff channel = %d\n",channel);
    mt_set_gpio_dir(GPIO_SPEAKER_EN_PIN,GPIO_DIR_OUT); // output
...
{
	PRINTK("jacob AudioDevice_Suspend\n");
	if(gsk_on)
	{
